import fs from "fs"
async function  writeSqlCms (obj){
  const json = JSON.stringify(obj)
  const string = `CREATE PROCEDURE [dbo].[cms]
...
begin
        ...
        select 'cms','content', ''
        union all
		select 'cms','page_il', '${json.replace(/'/g, '\"')}'
        union all
        ...
        return await fs.promises.writeFile("cms.sql",string)
}
writeSqlCms({
  $type: "group",
  core: "group",
