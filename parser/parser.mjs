...
class HTMLParser {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (let i = 0; i < resultArray.length; i++) {
          ...
          if (item.basis === true && item.content && item.content.length > 0) {
            content = await this.processTags(item.content);
    ...
    array.forEach(async (item) => {
      if (Array.isArray(item) && item.length > 0) {
        ...
        if (item.basis === true) {
          ...
          if (partResults == null) {
            throw new Error("Error in processBasisTags");
          }
          childrenArray.push(partResults);
        }
      }
  ...
  async processBasisTags(object) {
    try {
      ...
      const basicResult = JSON.parse(JSON.stringify(result));
      const basicContent = JSON.parse(JSON.stringify(object.content));
      result = this.processBasisContent(
        object.content,
        this.configObj[coreForError].Elements,
        result
      );
      if (Array.isArray(result)) {
        await this.processBasisContent(
          basicContent,
          this.configObj[coreForError].Elements,
          basicResult
        );
  ...
  processBasisContent(content, elements, il) {
    let tempString = "";
    ...
    content.forEach(async (item) => {
      ...
      if (content.basis === true) {
        ...
        if (item.type == "text") {
            ...
            if (item.name.startsWith("/")) {
              if (
                ...
                if (elements[item.name.slice(1)].TO) {
                  if (elements[item.name.slice(1)].Multi == true) {
          ...
          if (elements[item.name].Multi == true) {
            if (elementMultiState[item.name] == 1) {
              if (il[item.name].length == 1) {
                throw new Error(
...
module.exports = HTMLParser.parse;
