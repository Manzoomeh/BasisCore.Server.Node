...
describe("Print command when have data", () => {
      ...
      context.addSource(
        new JsonSource(
          [
            ...
            { id: 6, name: "naser", lastname: null },
            { id: 7, name: "", lastname: "zeinali" },
          ],
          "clients.names"
        )
      );
  ...
  test("result must </tr><tr> have devider", async () => {
    ...
    expect(result._result.split("</tr><tr>").length).toBe(4);
  });
  ...
  test("check replace command in one expression", async () => {
    il.faces.push({
      ...
      // "row-type": "odd",
      filter: "id<2",
      content: "<td style='color:blue' id='@id'>[(i)6]<p>--  @name<br></td>",
    });
    const print = new PrintCommand(il);
    const result = await print.executeAsync(context);
  ...
  test("dont replace when there is no match", async () => {
    il.faces.push({
      ...
      // "row-type": "odd",
      filter: "id<2",
      content: "<td style='color:blue' id='@id'>[(k)5]<p>--  @name<br></td>",
    });
    const print = new PrintCommand(il);
    const result = await print.executeAsync(context);
    expect(result._result).toBe(
...
describe("multi faces conditions", () => {
  ...
  test("check which multi faces applies in specefic condition; first condition should be applied only", async () => {
    il.faces.push(
      ...
      },
      {
        ...
        //"row-type": "odd",
        filter: "id<2",
        content: "<td style='color:blue-odd' id='@id'><p>--  @name<br></td>",
      }
    );
    const print = new PrintCommand(il);
    const result = await print.executeAsync(context);
  ...
  test("show one null field", async () => {
    ...
    const result = await print.executeAsync(context);
    expect(result._result).toContain(
      "<td style='color:green' id='6'><p>--  naser <br></td>"
    );
  });
  test("show one blank field", async () => {
    //this value may not be acceptable
    il.faces = [
      {
        name: "face1",
    ...
    const result = await print.executeAsync(context);
    expect(result._result).toContain(
      `<td style='color:green' id='7'><p>--   zeinali<br></td>`
    );
  });
  test("result must shown i group command", async () => {
    il.faces.push({
      name: "face1",
      replace: true,
      function: true,
    ...
    const print = new GroupCommand(groupIl);
