...
export default class HalfDonutChart {
  ...
  constructor(data, chartSetting, chart, dom) {
    ...
    this.dom = dom;
    this.radius = this.chartSetting.style.height > this.chartSetting.style.width ? this.chartSetting.style.height : this.chartSetting.style.width / 2
  ...
  renderChart() {
    ...
    if (chartContent) {
      this.chart.append("foreignObject").attr('x', (d, i) => {
        return this.radius - (((this.radius - (innerRadiusDistance || 70)) * Math.sqrt(2)) / 2)
      })
        .attr('y', (d) => {
          return height - ((this.radius - (innerRadiusDistance || 70)) * Math.sqrt(2) / 2) + 10
        })
        .attr('width', (this.radius - (innerRadiusDistance || 70)) * Math.sqrt(2) - 10)
        .attr('height', ((this.radius - (innerRadiusDistance || 70)) * Math.sqrt(2) / 2) - 10).html(chartContent)
    }
  }
  applyFeatures() {
